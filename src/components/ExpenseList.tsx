...
export default function ExpenseList() {
  ...
  const convertToUSD = useCallback(
    async (amount: number, originalCurrency: string): Promise<number> => {
      if (originalCurrency === 'USD') return amount;

      try {
        const url = `https://api.exchangerate.host/convert?access_key=${API_KEY}&from=${originalCurrency}&to=USD&amount=${amount}&format=1`;
  ...
  // Convert all expenses after loading
  useEffect(() => {
    const fetchConvertedExpenses = async () => {
      const updatedExpenses = await Promise.all(
        expenses.map(async (expense) => {
